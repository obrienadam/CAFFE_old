...
BOOST_AUTO_TEST_CASE (test1)
{
    ...
    BOOST_REQUIRE_EQUAL(mesh.nCells(), 64*64*64/8);
...
BOOST_AUTO_TEST_CASE (test3)
{
    ...
    for(int i = 0; i < 6; ++i)
    {
        if((*mesh.getAdjProcNoPtr())[i] != Parallel::PROC_NULL)
            BOOST_REQUIRE_EQUAL(bcs.getType(i), PrimitiveBoundaryCondition<double>::PARALLEL);
        else
            BOOST_REQUIRE_EQUAL(bcs.getType(i), PrimitiveBoundaryCondition<double>::FIXED);
    }
    ...
    for(int k = 0; k < phi.sizeK(); ++k)
    {
        for(int j = 0; j < phi.sizeJ(); ++j)
        {
            if((*mesh.getAdjProcNoPtr())[0] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(phi.sizeI(), j, k), (*mesh.getAdjProcNoPtr())[0]);
            if((*mesh.getAdjProcNoPtr())[1] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(-1, j, k), (*mesh.getAdjProcNoPtr())[1]);
        }
    }
    ...
    for(int k = 0; k < phi.sizeK(); ++k)
    {
        for(int i = 0; i < phi.sizeI(); ++i)
        {
            if((*mesh.getAdjProcNoPtr())[2] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(i, phi.sizeJ(), k), (*mesh.getAdjProcNoPtr())[2]);
            if((*mesh.getAdjProcNoPtr())[3] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(i, -1, k), (*mesh.getAdjProcNoPtr())[3]);
        }
    }
    ...
    for(int j = 0; j < phi.sizeJ(); ++j)
    {
        for(int i = 0; i < phi.sizeI(); ++i)
        {
            if((*mesh.getAdjProcNoPtr())[4] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(i, j, phi.sizeK()), (*mesh.getAdjProcNoPtr())[4]);
            if((*mesh.getAdjProcNoPtr())[5] != Parallel::PROC_NULL)
                BOOST_REQUIRE_EQUAL(phi(i, j, -1), (*mesh.getAdjProcNoPtr())[5]);
        }
    }
}
